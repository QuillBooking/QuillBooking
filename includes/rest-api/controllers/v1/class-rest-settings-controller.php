...
class REST_Settings_Controller extends REST_Controller
{
		...
		register_rest_route(
			...
			array(
				...
				array(
					...
					'args' => $this->get_endpoint_args_for_item_schema(WP_REST_Server::CREATABLE, $this->get_item_schema()),
				),
			)
		);
	}
	...
	{ // phpcs:ignore
		...
		$result = array();
		foreach ($this->get_item_schema()['properties'] as $group_key => $group_schema) {
