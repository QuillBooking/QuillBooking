...
class Updater {
	...
	private function __construct() {
		...
		add_action(
			...
			function() {
				update_option( 'quillbooking_pro_update_cache_needs_clear', true );
			}
		);
		if ( get_option( 'quillbooking_pro_update_cache_needs_clear' ) ) {
			update_option( 'quillbooking_pro_update_cache_needs_clear', false );
	...
	public function filter_plugins_api( $result, $action, $args ) {
		...
		$updates_data       = $this->get_pro_update();
		$plugin_update_data = quillbooking_objects_find( $updates_data, 'slug', $args->slug );
	...
	private function get_pro_update() {
		...
		$cache_key = 'quillbooking_pro_updates';
		...
		foreach ( $response['data'] as $plugin_slug => $item ) {
			if ( 'quillbooking-pro' === $plugin_slug ) {
	...
	public function add_in_plugin_update_message() {
		...
		$license_page = esc_url( admin_url( 'admin.php?page=quillbooking&path=license' ) );

		// invalid license.
		if ( ! $license_info || 'valid' !== $license_info['status'] ) {
			echo '&nbsp;<strong><a href="' . esc_attr($license_page) . '">' . esc_html__( 'Enter valid license key for automatic updates.', 'quillbooking' ) . '</a></strong>';
			return;
		}
	}
	...
	public function clear_pro_update_cache() {
		...
		delete_transient( 'quillbooking_pro_updates' );
