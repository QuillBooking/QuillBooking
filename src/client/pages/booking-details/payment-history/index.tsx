		...
		<div className="border px-10 py-8 rounded-2xl flex flex-col gap-5">
			...
			<Flex vertical gap={10} className='pb-4 border-b mb-4'>
				...
				<Flex align='flex-start' className='text-[#71717A] text-base gap-48'>
					<Flex vertical>
						<div>{__(`Payment Total: ${booking?.order?.total}`, 'quillbooking')}</div>
						<div>{__(`Payment Method: ${booking?.order?.payment_method}`, 'quillbooking')}</div>
			...
			<div className="">
				<table className="w-full text-left text-gray-700">
					<thead className="bg-[#F3F4F6] text-[#09090B] font-medium">
						<tr>
							<th className="pl-5 pr-4 py-5 w-2/3">{__("Name", "quillbooking")}</th>
							<th className="pl-2 py-5 w-1/3">{__("Price", "quillbooking")}</th>
						</tr>
					</thead>
					<tbody>
						{booking?.order?.items?.map((item, index) => (
							<tr key={index} className="bg-white">
								<td className="py-4 pl-5 pr-4 w-2/3">{item.item}</td>
								<td className="pl-2 py-5 w-1/3 flex">{booking?.order?.currency} {item.price}</td>
							</tr>
						))}
					</tbody>
				</table>
				...
				<div className="flex justify-end mt-2">
					<div className="w-2/5 text-right pl-1 pr-56 flex">
						...
						<span className="font-medium text-[#0EAD69] flex">{booking?.order?.currency} {booking?.order?.total}</span>
					</div>
				</div>
			</div>
