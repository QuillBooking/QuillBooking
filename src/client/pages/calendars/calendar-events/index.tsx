...
const CalendarEvents: React.FC<{ calendar: Calendar; typesLabels: Record<string, string>; onDeleted: (id: number, calendarId: number) => void; onDuplicated: (event: Record<string, any>, calendarId: number) => void }> = ({ calendar, typesLabels, onDeleted, onDuplicated }) => {
    ...
    const handleDisableEvent = (event) => {
        setDisabledEvent(prev => ({
            ...prev,
            [event]: !prev[event]
        }));
    };
            ...
            {calendar.events.length > 0 ? (
                <div className="quillbooking-calendar-events">
                    {calendar.events.map((event) => {
                        const isDisabled = disabledEvent[event.id];
                        return (
                            <Card
                                key={event.id}
                            ...
                            >
                                <Flex gap={20} vertical>
                                    <Flex justify="space-between" className='border-b pb-2'>
                                        <Flex vertical gap={2}>
                                            <Typography.Title level={5} style={{ margin: 0 }} className='capitalize text-[16px] font-[700] text-[#313131]'>{event.name}</Typography.Title>
                                            <Typography.Text type="secondary" className='text-[#1A1A1A99] text-[14px] font-[400] flex items-center gap-2'>
                                                <TimeIcon />
                                                {event.duration} {__('Mins', 'quillbooking')}
                                            </Typography.Text>
                                        </Flex>
                                        <Popover
                                            trigger={['click']}
                                            ...
                                            <div className='flex flex-col'>
                                                ...
                                                <span className="text-[#09090B] text-[14px] font-[500] capitalize">
                                                    {event.location.map((loc, index) => (
                                                        <span key={index}>
                                                            {loc.type}{index !== event.location.length - 1 && ", "}
                                                        </span>
                                                    ))}
                                                </span>
                                            </div>
                                ...
                                <Typography.Link href={`${siteUrl}?quillbooking_event=${event.slug}`} target='_blank'>
                                    ...
                                    <Flex justify="space-between" className='border-t pt-3'>
                                        <Button
                                            icon={<CloneIcon />}
                                            type="text"
