	...
	return (
		<div className="flex gap-4">
			<div className="flex-shrink-0">
				<div className="flex flex-col justify-center items-center rounded-xl size-14 bg-gray-200 animate-pulse" />
			</div>
			<div className="w-full border-dashed border-b-2 border-[#DEE1E6] pb-2 mb-2">
				<div className="space-y-4">
					<Flex align="center" className="pb-2 w-full">
						<Flex
						...
						>
							...
							<div className="size-8 bg-gray-200 animate-pulse rounded-lg" />
						</Flex>
					</Flex>
				</div>
			</div>
...
const LatestBookings: React.FC = () => {
	...
	const author = currentUser.isAdmin() ? 'all' : 'own';
			...
			<Card>
				...
				{loading ? (
					<div className="space-y-6">
						{[1, 2, 3].map((index) => (
							<BookingShimmer key={index} />
						))}
					</div>
				) : Object.keys(bookings).length > 0 ? (
					<>
						{Object.entries(bookings).map(
							([dateLabel, bookings], index, array) => {
									...
									<div className="flex gap-4" key={dateLabel}>
										<div className="flex-shrink-0">
											...
											>
												...
												<span
													className={`text-2xl font-bold ${isTodayDate ? 'text-white' : 'text-color-primary'}`}
												>
													{number}
												</span>
											</div>
					...
					<div className="flex flex-col gap-4 justify-center items-center mt-4 h-full p-4 my-6 py-6">
						<div className="w-36 h-36 flex justify-center items-center rounded-full bg-[#F4F5FA] border border-solid borderColor-[#E1E2E9] text-[#BEC0CA]">
							<BookingIcon width={60} height={60} />
						</div>
