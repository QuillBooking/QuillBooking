...
const EventAdvancedSettings = forwardRef<
...
>((props, ref) => {
	...
	const fetchSettings = () => {
		...
		callApi({
			...
			onSuccess(response: AdvancedSettings) {
				...
				setInitialSettings(JSON.parse(JSON.stringify(response)));
				// Update both slug states with current event slug
				setSlug(event.slug || '');
	...
	// Effect to handle initial load and event changes
	useEffect(() => {
		if (!event) return;
	...
	// Effect to sync slug with event changes
	useEffect(() => {
		if (event?.slug) {
			setSlug(event.slug);
			setInitialSlug(event.slug);
		...
		setSettings((prev) => {
			...
			return {
				...
				permission_denied_message:
					prev.permission_denied_message + mention,
			};
		});
		setActiveModalType(null);
		checkForChanges();
					...
					<Flex vertical gap={20}>
						...
						<Card className="rounded-lg mt-4">
							<Flex vertical gap={16}>
								<Skeleton.Input active block className="h-6" />
								<Skeleton.Input active block className="h-12" />
								<Skeleton.Input active block className="h-6" />
								<Skeleton.Input active block className="h-12" />
							</Flex>
						</Card>
		...
		<div className="grid grid-cols-2 gap-5 px-9">
				...
				<Card className="rounded-lg mt-4">
					<Flex vertical>
						...
						<Input
							...
							placeholder={__(
								'{event name} between {host name} and {{guest.first_name}}',
								'quillbooking'
							)}
							className="h-[48px] rounded-lg"
							onChange={(e) =>
								handleChange('event_title', e.target.value)
							}
							suffix={
								<span
									className="bg-[#EEEEEE] p-[0.7rem] rounded-r-lg"
									onClick={() =>
										setActiveModalType('bookingTitle')
									}
								>
									<UrlIcon />
								</span>
			...
			<Card>
				<Flex vertical gap={20}>
					<Card className="rounded-lg">
						<FieldWrapper
							label={
								<span className="text-[#09090B] text-[20px]">
									{__(
							...
							description={
								<span className="text-[#71717A] text-[14px]">
									{__(
										'Redirect user to a custom URL after a successful booking',
										'quillbooking'
									)}
								</span>
							}
							...
							<>
								...
								>
									<Input
										...
										size="large"
										placeholder={__(
											'https://example.com/redirect-to-my-success-page',
											'quillbooking'
										)}
										className="rounded-lg h-[48px]"
										suffix={
									...
									{passingFields && (
										<>
											...
											<span className="text-[#71717A] text-[14px] italic">
												{__(
													'Sample: email={{guest.email}}&phone={{booking.phone}}',
													'quillbooking'
												)}
											</span>
										</>
					...
					<Card className="rounded-lg">
						<FieldWrapper
							...
							description={
								<span className="text-[#71717A] text-[14px]">
									{__(
										'Please ensure the cancel link is included in the confirmation email.',
										'quillbooking'
									)}
								</span>
							}
								...
								<Flex gap={10} vertical>
									<FieldWrapper>
										...
										>
											...
											>
												{__(
													'When meeting starts in less than',
													'quillbooking'
												)}
											</Radio>
										</Radio.Group>
									...
									<FieldWrapper
										...
										description={
											<span className="text-[#71717A] text-[14px] italic">
												{__(
													'User will see this if they attempt to cancel without permission.',
													'quillbooking'
												)}
											</span>
										}
									>
										<Input
											value={
												settings.permission_denied_message
											}
											onChange={(e) =>
												handleChange(
													'permission_denied_message',
					...
					<Card className="rounded-lg">
						<FieldWrapper
							...
							description={
								<span className="text-[#71717A] text-[14px]">
									{__(
										'Update the slug to customize your event landing page URL',
										'quillbooking'
									)}
								</span>
							}
						...
						>
							<div
								className="border border-[#EDEBEB] rounded-md cursor-pointer h-fit p-2"
						...
						{showSlugField && (
							<div className="pt-4">
								<FieldWrapper
									label={
										<span className="text-[#09090B] text-[14px] font-semibold">
									...
									description={
										<span className="text-[#71717A] text-[14px]">
											{__(
												'Slug must be unique to avoid any conflicts with other events',
												'quillbooking'
											)}
										</span>
									}
			...
			>
				<Flex gap={10} className="items-center border-b pb-4 mb-4">
					...
					<Header
						header={
							activeModalType === 'bookingTitle'
								? __('Booking Title Merge tags', 'quillbooking')
								: activeModalType === 'redirectUrl'
									? __('Redirect URL', 'quillbooking')
				...
				<MergeTagModal
					onMentionClick={
						activeModalType === 'bookingTitle'
							? handleBookingTitleClick
							: activeModalType === 'redirectUrl'
								? handleRedirectUrlClick
								: activeModalType === 'queryString'
									? handleQueryStringClick
