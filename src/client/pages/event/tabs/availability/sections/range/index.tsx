    ...
    return (
        <Flex vertical gap={10} className='mt-5'>
            <Text className='text-[#3F4254] text-base font-semibold'>
                ...
                <span className='text-[#71717A] italic text-base font-normal pl-1'>{__("(Invitees can schedule)", "quillbooking")}</span>
            </Text>
            <Radio.Group
                value={range.type}
            ...
            >
                <Radio
                    value="days"
                    className={`border rounded-lg py-4 px-3 text-[#3F4254] font-semibold custom-radio ${range.type === "days" ? "border-color-primary bg-color-secondary" : ""
                        }`}
                >
                ...
                <Radio
                    value="date_range"
                    className={`border rounded-lg py-4 px-3 text-[#3F4254] font-semibold custom-radio ${range.type === "date_range" ? "border-color-primary bg-color-secondary" : ""
                        }`}
                >
                ...
                <Radio
                    value="infinity"
                    className={`border rounded-lg py-4 px-3 text-[#3F4254] font-semibold custom-radio ${range.type === "infinity" ? "border-color-primary bg-color-secondary" : ""
                        }`}
                >
            ...
            {range.type === 'date_range' && (
                ...
                <Flex gap={20} className='mt-4'>
                    <DatePicker
                        ...
                        onChange={(date) => {
                            if (date) {
                                onDateRangeChange(date.format('YYYY-MM-DD'), range.end_date ?? '');
                            }
                        }}
                    ...
                    <DatePicker
                        ...
                        onChange={(date) => {
                            if (date) {
                                onDateRangeChange(range.start_date ?? '', date.format('YYYY-MM-DD'));
                            }
                        }}
