 ...
 */
const OAuthIntegrations: React.FC<{
	...
	if (integrations.length === 0) {
		return (
			<div className="p-4 text-center">
				<Text type="secondary">
					{__('No OAuth integrations available', 'quillbooking')}
				</Text>
			</div>
		);
	}
	...
	return (
		<Row gutter={[24, 24]}>
			{integrations.map(([slug, integration]) => (
				<Col xs={24} key={slug}>
					<IntegrationCard slug={slug} integration={integration} />
				</Col>
			))}
		</Row>
	);
...
const IntegrationCard: React.FC<{
	...
	useEffect(() => {
		const appSettings: { use_custom_app?: boolean } =
			integration.settings.app || {};
		form.setFieldsValue(appSettings);
		setUseCustomApp(!!appSettings.use_custom_app);
	}, [integration, form]);
				...
				<div style={{ flexGrow: 1 }}>
					...
					>
						...
						>
							<Checkbox
								onChange={(e) =>
									setUseCustomApp(e.target.checked)
								}
							>
								{__(
									'Use custom app credentials',
									'quillbooking'
								)}
							</Checkbox>
						</Form.Item>
