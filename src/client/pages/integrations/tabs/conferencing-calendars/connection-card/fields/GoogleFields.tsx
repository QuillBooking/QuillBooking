...
const GoogleFields = ({ CACHE_TIME_OPTIONS, form, calendar }) => {
	...
	useEffect(() => {
		console.log('Google cache_time:', cacheTime);
	}, [cacheTime]);

	const handleCacheTimeChange = (value) => {
		console.log('Google selection changed to:', value);
	...
	const handleNavigateToIntegration = async () => {
		...
		try {
			...
			navigate(
				`/wordpress/wp-admin/admin.php?page=quillbooking&path=${encodeURIComponent(path)}&tab=integrations&subtab=google`
			);
		...
		<div className="google-fields">
			<Flex vertical gap={10} className="w-full">
				...
				>
					...
					<div className="text-[#71717A] italic">
						{__(
							'Select how many minutes the Google Calendar / Meet events API call will be cached...',
							'quillbooking'
						)}
					</div>
					<Button
