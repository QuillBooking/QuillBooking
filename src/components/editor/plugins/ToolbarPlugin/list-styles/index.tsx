...
export default function ListStyles({ activeEditor }: ListStylesProps) {
  ...
  const toggleList = (command, listType) => {
    if ((listType === 'bullet' && isBulletList) || 
  ...
  return (
    <Flex gap={10}>
      <Button
        onClick={() => toggleList(INSERT_UNORDERED_LIST_COMMAND, 'bullet')}
        title="Bullet List"
      ...
      <Button
        onClick={() => toggleList(INSERT_ORDERED_LIST_COMMAND, 'number')}
        title="Numbered List"
        className='border-none shadow-none cursor-pointer p-0'
      >
        <FaListOl className={`text-[20px] ${
      ...
      <Button
        ...
        className='border-none shadow-none cursor-pointer p-0'
      >
        <MdOutlineChecklist className={`text-[20px] ${
