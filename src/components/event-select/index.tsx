...
const EventSelect: React.FC<EventSelectProps> = ({ value, onChange, multiple = false, placeholder, exclude, type = 'event' }) => {
    ...
    const fetchEvents = async (input = '', ids: number[] = []) => {
        ...
        return new Promise((resolve) => {
            callApi({
                path: addQueryArgs(`calendars`, { per_page: 10, ...data, filters: { type } }),
                method: 'GET',
                onSuccess: (response: CalendarResponse) => {
                    const newEvents = response.data.flatMap((calendar) => calendar.events);
