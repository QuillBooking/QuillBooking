...
const DEFAULT_DESCRIPTION = __("Select one of Merge tags that related to your input.", "quillbooking");
...
const MergeTagModal: React.FC<MergeTagProps> = ({ onMentionClick }) => {
  ...
  const [selectedKey, setSelectedKey] = useState(groupKeys.length > 0 ? groupKeys[0] : "");
  ...
  const TagCard = ({ name, value, category }) => (
    ...
    >
      <Flex vertical gap={3}>
        <span className="italic text-[#3F4254] text-[16px] font-semibold">{name}</span>
      ...
      <Card className="w-[450px]">
        <Flex vertical gap={10}>
          {groupKeys.map((groupKey) => {
            ...
            const description = group.description || DEFAULT_DESCRIPTION;
            // Get appropriate icon or fallback to the "other" icon
            const icon = CATEGORY_ICONS[groupKey.toLowerCase()] || <MergeTagOtherIcon />;
      ...
      <Card className="w-[450px]">
        <Flex vertical gap={15}>
          {selectedKey && mergeTagGroups[selectedKey]?.mergeTags ? (
            Object.entries(mergeTagGroups[selectedKey].mergeTags).map(([tagKey, tag], index) => (
              <TagCard 
                key={index}
                name={tag.name}
                value={tag.value}
