...
const Reschedule: React.FC<RescheduleProps> = ({
  ...
  const handleFinish = async () => {
    ...
    //   formData.append('reason', values.reasonForReschedule);
    // }
    ...
    try {
      ...
      if (response.ok) {
        ...
        (window.top || window).location.href = `${baseUrl}/?quillbooking=booking&id=${booking?.hash_id}&type=confirm`;
      }
    } catch (error) {
        ...
        >
          ...
          {reschedulingReasonField && (
            <Form.Item
              ...
              rules={[
                {
                  required: reschedulingReasonField.required,
                  message: __('Please provide a reason for rescheduling', 'quillbooking'),
                },
              ]}
            >
              <TextArea
                placeholder={reschedulingReasonField.placeholder}
              />
            </Form.Item>
          )}
